...
// Emitted once whenever the state flips from one variant to another.
// PlayerState / Player are already in this file (chapter 6).
#[derive(Message)]
...
fn player_changed_state(
    ...
    mut ev_changed_state: MessageWriter<PlayerChangedState>,
...
fn update_animations(
    ...
    let (player, mut animator) = match q_player.single_mut() {
        Ok(r) => r,
        Err(_) => return,
    };
...
fn adjust_sprite_flip(
    mut q_player: Query<(&mut Sprite, &Player)>,
) {
    ...
    //Don't flip mid-dash/hook; otherwise face the travel direction.
    if player.state == PlayerState::Hooking || player.state == PlayerState::Dashing {return;}
    if player.current_direction.x == 0.0 {return;}
    sprite.flip_x = player.current_direction.x < 0.0;
}

fn leave_dash(mut q_player: Query<(&mut Player, &AnimationPlayer2D)>) {
    let (mut player, animator) = match q_player.single_mut() {
        Ok(r) => r,
        Err(_) => return,
    };

    if player.state == PlayerState::Dashing && animator.is_finished() {
...
impl Plugin for PlayerStatePlugin {
    fn build(&self, app: &mut App) {
        app.add_message::<PlayerChangedState>()
            .add_systems(
                    PostUpdate,
                    (
